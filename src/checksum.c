...
static inline void
ipv4_pseudo_header_init (ipv4_pseudo_header_t *hdr, uint32_t source_ip,
                         uint32_t dest_ip, uint8_t proto, uint16_t tcp_length)
{
...
uint32_t
sum_16bit_words (buffer_t *buffer)
{
...
__sum16
ipv4_checksum (ipv4_pseudo_header_t *pseudo_header, buffer_t *packet)
{
...
__sum16
tcp_checksum (buffer_t *packet, uint32_t source_ip, uint32_t dest_ip)
{
  uint16_t tcp_length = 0;
  ipv4_pseudo_header_t pseudo_header = { 0 };

  tcp_length = packet->len;
  ipv4_pseudo_header_init (&pseudo_header, source_ip, dest_ip, IPPROTO_TCP,
                           packet->len);
